...
try:
    readme = urllib.request.urlopen('https://github.com/metaMMA/metaMMA/blob/master/README.md').read().decode(
        'utf-8')
...
if StrictVersion(latest_version_num) > StrictVersion(version):
    logger.info(
        "v" + latest_version_num + " is an update you your current version (v" + version + "). Attempting to dowload from github.com.")
    ...
    if os.path.isdir(os.path.join(os.path.join(info_check.home, ".metaMMAold"),
                                  "")):  # if the previous metaMMA version is stored, delete it
    ...
    move(info_check.meta, os.path.join(os.path.join(info_check.home, ".metaMMAold"),
                                       ""))  # rename the current metaMMA release to 'metalMMAold'
