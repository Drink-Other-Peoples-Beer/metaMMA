...
base64string = base64.encodebytes(
    ('%s:%s' % (user_info.plex_username, user_info.plex_password)).encode()).decode().replace('\n', '')
txdata = ""
...
section_info_xml = urllib.request.urlopen(
    'http://' + user_info.plex_ip + ':32400/library/sections/?X-Plex-Token=' + token_str).read().decode('utf-8')
